...
| 表 | 实际单据 | display_number | source_doc_id | sales_contract_id | root_sales_contract_id | customer_id |
|---|---|---|---|---|---|---|
| `inquiries` | ING | ❌（用 `inquiry_number`） | ❌ | ❌ | ❌ | ❌ |
| `sales_quotations` | QT | ❌（用 `qt_number`） | ❌ | ❌ | ❌ | ❌ |
| `sales_contracts` | SC | ❌（用 `contract_number`） | ❌ | ✅（= self） | ❌ | ❌ |
| `purchase_requirements` | **QR**（非PR） | ❌（用 `requirement_number`） | ❌ | ❌ | ❌ | ❌ |
| `quotation_requests` | QR（旧，待确认） | ❌（用 `request_number`） | ❌ | ❌ | ❌ | ❌ |
| `supplier_xjs` | XJ | ❌（用 `xj_number`） | ❌ | ❌ | ❌ | ❌ |
| `supplier_quotations` | BJ | ❌（用 `xj_number`引用） | ❌ | ❌ | ❌ | ❌ |
| `purchase_orders` | CG | ❌（用 `po_number`） | ❌ | ❌ | ❌ | ❌ |
| `payments` | Finance | ❌（用 `payment_number`） | ❌ | ❌ | ❌ | ❌ |
| PR表 | **PR**（缺失） | ❌ 表不存在 | ❌ | ❌ | ❌ | ❌ |
...
- **新建 `purchase_requests` 表**（PR，SC→CG 的采购请求，修复缺失环节）
