...
export const PurchaseOrderEditDialog: React.FC<PurchaseOrderEditDialogProps> = ({
  ...
  const [basicSectionOpen, setBasicSectionOpen] = useState(false);
  ...
  const renderReadOnlyField = (label: string, value: string, className = '') => (
    <div className={`space-y-1 ${className}`.trim()}>
      ...
      <Input value={value || '-'} readOnly className="h-8 text-xs bg-gray-50 text-gray-600" />
    </div>
  );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="max-w-none max-h-[90vh] overflow-y-auto overflow-x-hidden p-0"
        style={{
          width: 'min(96vw, calc(210mm + 140px))',
          maxWidth: 'min(96vw, calc(210mm + 140px))',
        }}
      >
        ...
        <div className="px-8 py-6">
          <div className="space-y-0 min-w-0 pb-24">
            <section className="mb-6">
              <button
                type="button"
                onClick={() => setBasicSectionOpen((current) => !current)}
                className="mb-3 flex w-full items-center justify-between rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-left"
              >
                <div>
                  <h4 className="text-sm font-semibold text-gray-900">基础信息区</h4>
                  <p className="mt-1 text-xs text-gray-500">采购单基础编号、状态、币种和交期信息，按需展开查看或调整</p>
                </div>
                <ChevronDown className={`h-4 w-4 text-slate-500 transition-transform ${basicSectionOpen ? 'rotate-180' : ''}`} />
              </button>
            ...
            <section className="mb-6">
              ...
              >
                <div>
                  <h4 className="text-sm font-semibold text-gray-900">供应商信息区</h4>
                  <p className="mt-1 text-xs text-gray-500">供应商资料来自供应商库，此处仅展示，不在采购合同内手工录入</p>
                </div>
                <ChevronDown className={`h-4 w-4 text-slate-500 transition-transform ${supplierSectionOpen ? 'rotate-180' : ''}`} />
              </button>
              ...
              {supplierSectionOpen ? (
                <div className="space-y-3 rounded-md border border-slate-200 bg-white p-4">
                  ...
                  {renderReadOnlyField('联系人', editPOForm.supplierContact)}
                  {renderReadOnlyField('电话', editPOForm.supplierPhone)}
                  {renderReadOnlyField('供应商地址', editPOForm.supplierAddress)}

                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    {renderReadOnlyField('开户银行', editPOForm.supplierBankName)}
                    {renderReadOnlyField('账户名称', editPOForm.supplierBankAccountName)}
                    {renderReadOnlyField('银行账号', editPOForm.supplierBankAccountNumber)}
                    {renderReadOnlyField('收款币种', editPOForm.supplierBankCurrency)}
                    {renderReadOnlyField('SWIFT代码', editPOForm.supplierBankSwiftCode)}
                    {renderReadOnlyField('银行地址', editPOForm.supplierBankAddress, 'md:col-span-2')}
                  </div>
                </div>
              ) : null}
            </section>
            ...
            <section className="mb-6">
              <button
                type="button"
                onClick={() => setTradeSectionOpen((current) => !current)}
                className="mb-3 flex w-full items-center justify-between rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-left"
              >
                <div>
                  <h4 className="text-sm font-semibold text-gray-900">交易信息区</h4>
                  <p className="mt-1 text-xs text-gray-500">贸易术语、港口、结算条款和采购明细统一收纳在此，按需展开编辑</p>
                </div>
                <ChevronDown className={`h-4 w-4 text-slate-500 transition-transform ${tradeSectionOpen ? 'rotate-180' : ''}`} />
              </button>
            ...
            <section className="mb-6">
              <button
                type="button"
                onClick={() => setTermsSectionOpen((current) => !current)}
                className="mb-3 flex w-full items-center justify-between rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-left"
              >
                <div>
                  <h4 className="text-sm font-semibold text-gray-900">合同条款区</h4>
                  <p className="mt-1 text-xs text-gray-500">付款、交货、质保、法务等采购合同条款统一收纳在此，按需展开编辑</p>
                </div>
                <ChevronDown className={`h-4 w-4 text-slate-500 transition-transform ${termsSectionOpen ? 'rotate-180' : ''}`} />
              </button>
