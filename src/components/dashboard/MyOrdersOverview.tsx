...
import { useOptionalInquiry } from '../../contexts/InquiryContext';
...
export function MyOrdersOverview({ activeOrders, orderHistory, onNavigate, onTabChange }: MyOrdersOverviewProps) {
  const { user } = useUser();
  const inquiryContext = useOptionalInquiry();
  
  // Get inquiries for current user
  const inquiries = user && inquiryContext ? inquiryContext.getUserInquiries(user.email) : [];
