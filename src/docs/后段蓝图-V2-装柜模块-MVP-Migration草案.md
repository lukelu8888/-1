...
1. `loading_pools`
2. `loading_pool_items`
3. `container_type_specs`
4. `container_loading_solutions`
5. `container_loading_solution_containers`
6. `container_loading_solution_items`
...
不在本轮引入 PostgreSQL enum type，避免后续迁移演进成本过高。
...
- `quantity` 用 `NUMERIC(18,3)`，兼容未来非整数业务单位
- `cartons` 仍允许小数，是为了兼容 bundle / pallet / handling group 折算

### 1.3 业务快照字段

以下字段明确视为“业务快照字段”，不是主数据主键替代：
...
```json
{
  ...
  "score": 0.82,
  "reason": "预计载重超过推荐安全比例",
  "suggested_action": "拆分至下一柜或改用40HQ"
}
```
...
### 1.5 planned / actual 语义

`actual_*` 默认值可以为 `0`，但语义不能靠数值本身判断。
...
- `actual_weight_kg = 0`
  不代表“实际没装”
- 只有执行状态推进后，`actual_*` 才具有业务含义

### 1.6 分配剩余量

MVP 不额外建 remaining 字段，统一由 service 层计算：

- `remaining_cartons = pool_item.carton_count - 已分配cartons总和`
- `remaining_quantity = pool_item.quantity - 已分配quantity总和`
- `remaining_weight_kg = pool_item.total_weight_kg - 已分配weight总和`
- `remaining_cbm = pool_item.total_cbm - 已分配cbm总和`
...
- 上游：`consolidation_plans / cargo_receipts`
- 下游：`loading_pool_items / container_loading_solutions`

### 3.2 `loading_pool_items`
...
- `manual_lock_container_key` 替代 `manual_lock_container_id`
- 语义是“锁定到稳定业务键”
- 不直接依赖 solution container UUID 生命周期

本轮不做硬唯一：

- `uq_loading_pool_item_source` 暂不落库
- 由 service 层做去重策略

### 3.3 `container_type_specs`
...
- `container_type_spec_id` 是主引用
- `container_type_code` 是快照字段
- service 层必须保证二者一致：
  - 先按 `spec_id` 取当前 code
  - 保存 container 记录时同时写入 code 快照

### 3.4 `container_loading_solutions`
...
- 风险汇总全部结构化存入 `risk_summary`
- 统一使用约定 schema

### 3.5 `container_loading_solution_containers`
...
- 已正式落入本表
- 但“是否已执行”仍由 `planning_status / solution_status` 判断

### 3.6 `container_loading_solution_items`
...
- 多订单
- 多供应商
- 多 `shipment_batch_no / shipment_split_no`

### 4.2 `loadingPoolItemService`
...
- `getRemainingByPoolItem(poolItemId)`
- `getRemainingByPool(poolId)`

### 4.3 `containerTypeSpecService`
...
- 柜型推荐
- 柜型组合对比
- 输出 `estimation_summary / recommended_container_mix / risk_summary`

### 4.5 `containerAllocationEngine`

职责：

- 规则分柜
- 生成 container / item 结果
- 识别人工锁定
- 计算 remaining 分配

### 4.6 `containerLoadingSolutionService`

职责：

- 保存 solution 主记录
- 保存 containers/items
- baseline / parent / version 管理
- confirmed / executed 状态流推进

## 5. 当前结论

这份 migration 草案已吸收以下要求：
...
- `spec_id + code` 主从关系
- 风险 JSONB 统一 schema
- planned vs actual 结构化落表
- remaining 由 service 计算
- `line_no / item_seq`

下一步可以据此继续进入：

1. 真正的 SQL migration 文件
2. 对应 service 骨架实现
