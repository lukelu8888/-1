# 后段蓝图第二批 Migration 草案

## 1. 范围

本草案继续沿着后段蓝图推进，只覆盖以下内容：

- 主线A
  - `装柜执行 -> 出口要求判定 -> 报关/开船前文件准备`
- 主线B
...
- `064_create_export_requirements_and_finance_packets.sql`

如果将来拆分，也可以拆成：

- `064_create_export_requirement_checks.sql`
- `065_create_finance_compliance_packets.sql`
...
-- 8. Realtime
-- ============================================================
ALTER PUBLICATION supabase_realtime ADD TABLE public.export_requirement_checks;
ALTER PUBLICATION supabase_realtime ADD TABLE public.finance_compliance_packets;
ALTER PUBLICATION supabase_realtime ADD TABLE public.finance_packet_document_slots;
ALTER PUBLICATION supabase_realtime ADD TABLE public.finance_packet_files;
```
...
### 5.1 `documentation-service` 如何处理

现有 [documentation-service.ts](/Users/luke/Documents/New%20project%202/innoshop_react20260221/src/lib/services/documentation-service.ts) 保留事件驱动思路，但不再把它视为最终文件包模型。

后续升级方向：

- `createDocumentationTask(orderId)` 逐步演进为 `ensureFinanceCompliancePacket(context)`
- `document_instances` 作为兼容层保留
- 真实文件槽位以 `finance_packet_document_slots` 为准

### 5.2 `DocumentCenter` 如何处理

现有 [DocumentCenter.tsx](/Users/luke/Documents/New%20project%202/innoshop_react20260221/src/components/admin/DocumentCenter.tsx) 继续作为模板与文档输出引擎。

职责保留：

- `CG / CI / PL / Statement / Waiver` 模板输出
...
### 5.3 `FinanceManagement` 如何处理

现有 [FinanceManagement.tsx](/Users/luke/Documents/New%20project%202/innoshop_react20260221/src/components/admin/FinanceManagement.tsx) 继续作为财务门户。

后续新增：

- `Compliance Packets / 合规文件包` 标签
...
- `D07` 更新为 `linked` 或 `uploaded`

#### `国内集货`

- `D03` 更新
- 如果本方不承担国内运费，`D04` 进入自动生成

#### `报关/开船`

- `D05/D06/D09` 更新
- `D08/D10` 按费用承担方决定是否生成
...
- 主线A：仍围绕 `装柜后 -> 报关前判定`
- 主线B：进入 `Finance Compliance Packet` 真实骨架
